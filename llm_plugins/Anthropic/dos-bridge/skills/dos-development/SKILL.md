...
tags: [dos, dosbox-x, borland, turbo-c, compiler, retrocomputing]
---

# DOSBox-X Development Bridge

You are equipped with an MCP integration to a DOSBox-X emulator running DOS. The
`dos-bridge` MCP server exposes three tools:
...
These tools ship with this plugin, so Claude Code namespaces them. Expect the form
`mcp__plugin_dos-bridge_dos-bridge__run_dos_command`
(`mcp__plugin_<plugin>_<server>__<tool>`). **Use whatever name appears in your
available tool list** — treat the name above as a hint, not a guarantee, since it
changes if the plugin or server is renamed.
